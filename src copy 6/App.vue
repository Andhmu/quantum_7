  ...
  <div class="multiverse-app">
    ...
    <div class="observer-interface" v-if="store.isInitialized">
      ...
      <div class="stats-panel">
        <div class="stat-item">
          <span class="label">TIMELINE_COLLAPSES:</span>
          <span class="value">{{ store.totalCollapses }}</span>
        </div>
        <div class="stat-item">
          <span class="label">AWARENESS_LEVEL:</span>
          <div class="progress-bar">
            <div 
              class="progress-fill" 
              :style="{ width: (store.awareness * 100) + '%' }"
            ></div>
          </div>
          <span class="value">{{ (store.awareness * 100).toFixed(1) }}%</span>
        </div>
      </div>
