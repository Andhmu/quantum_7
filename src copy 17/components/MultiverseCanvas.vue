  ...
  <div class="multiverse-wrapper">
    ...
    <Transition name="slide">
      <div class="side-panel" v-if="activeWorld">
        ...
        <div class="footer">
          <div class="stat-item"><label>ВРЕМЯ_В_МИРЕ</label><span>{{ formatTime(store.currentWorldTime || 0) }}</span></div>
          <div class="stat-item"><label>УГРОЗЫ</label><span>{{ engine?.currentHazards?.length || 0 }} ЕД.</span></div>
          <div class="stat-item"><label>КООРДИНАТЫ</label><span>{{ Math.round(activeWorld.x || 0) }}:{{ Math.round(activeWorld.y || 0) }}</span></div>
        </div>
      </div>
    </Transition>
  </div>
